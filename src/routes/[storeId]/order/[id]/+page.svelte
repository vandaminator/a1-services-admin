    ...
    modal.response = (confirm: boolean) => {
      if (confirm) {
        const apiDelete = async () => {
          loading = true;
          const response = await fetch(`/${storeId}/api/order/${info.id}`, {
            method: "DELETE",
          });
  ...
  const openDoneModal = () => {
    ...
    modal.response = (confirm: boolean) => {
      if (confirm) {
        const apiDelete = async () => {
          loading = true;
          const response = await fetch(`/${storeId}/api/order/${info.id}?isDone=true`, {
            method: "PATCH",
          });
  ...
  const openDeliveredModal = () => {
    ...
    modal.response = (confirm: boolean) => {
      if (confirm) {
        const apiDelete = async () => {
          loading = true;
          const response = await fetch(`/${storeId}/api/order/${info.id}?delivered=true`, {
            method: "PATCH",
          });
